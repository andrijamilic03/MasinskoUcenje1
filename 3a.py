...
np.random.seed(42) # Nije neophodno, ali omogocuava da se pri svakom pokretanju dobijaju isti rezultati
...
knn = KNN(nb_features=2, nb_classes=3, data={'x': X_train, 'y': y_train}, k=3)
...
h = .02  # Veličina koraka za mesh grid
x_min, x_max = X[:, 0].min() - 1, X[:, 0].max() + 1
y_min, y_max = X[:, 1].min() - 1, X[:, 1].max() + 1

xx, yy = np.meshgrid(np.arange(x_min, x_max, h), np.arange(y_min, y_max, h))
Z = np.array([knn.predict({'x': np.array([[x, y]])}) for x, y in zip(np.ravel(xx), np.ravel(yy))])
Z = Z.reshape(xx.shape)

plt.contourf(xx, yy, Z, alpha=0.3)  # Klasifikacione oblasti

colors = ['red', 'blue', 'green']
for class_idx, color in enumerate(colors):
    plt.scatter(X_train[y_train == class_idx, 0], X_train[y_train == class_idx, 1], c=color,
                label=list(class_mapping.keys())[class_idx])
plt.xlabel('Feature 1')
plt.ylabel('Feature 2')
plt.legend()
plt.title('KNN klasifikacija')
plt.show()
